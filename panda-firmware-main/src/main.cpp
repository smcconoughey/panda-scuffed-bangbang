 ...
 *   'EVT:<ms>:<cat>:<L|F>:<detail>'                audit event on every BB transition
 *
...
BBController bbLox (v2PtData, BB_LOX_PT_CH,
                    BB_LOX_DC_CH,  BB_LOX_VENT_DC_CH,
                    BB_LOX_VENTURI_UP_PT, BB_LOX_VENTURI_DN_PT,  'L');
BBController bbFuel(v2PtData, BB_FUEL_PT_CH,
                    BB_FUEL_DC_CH, BB_FUEL_VENT_DC_CH,
...
static void handleB(const char* pkt) {   // core config
    ...
    float sp, db; unsigned long wt, maxOpen;
...
static void handleV(const char* pkt) {   // auto-vent config
    ...
    BBController* ctrl = pickSide(pkt[1]);
    if (!ctrl)          { Serial2.println("BB_ERROR:bad_side"); return; }
    float trig; int autoOn;
    if (sscanf(pkt + 2, "%f,%d", &trig, &autoOn) != 2 || (autoOn != 0 && autoOn != 1)) {
        Serial2.println("BB_ERROR:parse");
        return;
    }
...
static void handleLowerB(const char* pkt) {  // enable/disable sustain
    if (strlen(pkt) < 3) { Serial2.println("BB_ERROR:short"); return; }
    BBController* ctrl = pickSide(pkt[1]);
    if (!ctrl)           { Serial2.println("BB_ERROR:bad_side"); return; }
    char stCh = pkt[2];
    if (stCh == '1') {
        if (!gArmed) { Serial2.println("BB_ERROR:not_armed"); return; }
        ctrl->enableSustain();
    } else if (stCh == '0') {
        ctrl->disableSustain();
    } else {
...
static void handleLowerX(const char* pkt) {  // latched abort
    if (strlen(pkt) < 2) { Serial2.println("BB_ERROR:short"); return; }
    BBController* ctrl = pickSide(pkt[1]);
    if (!ctrl)           { Serial2.println("BB_ERROR:bad_side"); return; }
    ctrl->latchAbort();
}
...
static const char* stateStr(BBState s) {
    switch (s) {
        case BBState::DISABLED:  return "OFF";
        case BBState::SUSTAIN:   return "SUS";
        case BBState::AUTO_VENT: return "AV";
        case BBState::ABORT:     return "ABT";
    }
    return "??";
}
...
static void printBbHeartbeat(const BBController& c) {
    ...
    Serial2.print(':'); Serial2.print(stateStr(c.state()));
    Serial2.print(':'); Serial2.print(c.isPressOpen() ? 1 : 0);
    Serial2.print(':'); Serial2.print(c.isVentOpen()  ? 1 : 0);
    Serial2.print(':'); Serial2.println(c.lastPressure(), 1);
}
...
void loop() {
  ...
  if (th.isPacketReady()) {
    ...
    if (idChar == 's') {
      ...
      else channel = 10 + (toupper(channelChar) - 'A');
      state = stateChar - '0';
    ...
    // Bang-bang commands
    else if (idChar == 'B') handleB(rxPacket);
    else if (idChar == 'V') handleV(rxPacket);
    else if (idChar == 'M') handleM(rxPacket);
    else if (idChar == 'b') handleLowerB(rxPacket);
    else if (idChar == 'v') handleLowerV(rxPacket);
    else if (idChar == 'x') handleLowerX(rxPacket);
